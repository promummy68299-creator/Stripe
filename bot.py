...
# ============================================
BOT_TOKEN = "8647485045:AAFWLQW9-IUd1jiDDESXyx-pGowiAX1rT3A"  # CHANGE THIS - Get from @BotFather
ADMIN_IDS = [8279891640]  # CHANGE THIS - Your Telegram user ID
...
# Detect which version we're using
...
`/setdelay [sec]` - Set check delay
`/toggleproxy` - Toggle proxy
`/setkey [pk]` - Set Stripe key
...
"""
    ...
    def chk_command(update, context):
        args = context.args
        if not args:
            update.message.reply_text("❌ Usage: `/chk 4111111111111111|12|2026|123`", parse_mode="Markdown")
            return
        
        msg = update.message.reply_text("🔥 Checking...")
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(checker.check_card(' '.join(args)))
        loop.close()
...
"""
    ...
    def mass_command(update, context):
        ...
        cards = load_cards()
        if not cards:
            update.message.reply_text("❌ No cards!")
            return
        
        msg = update.message.reply_text(f"🔥 Checking {len(cards)} cards...")
        ...
        for i, card in enumerate(cards):
            result = loop.run_until_complete(checker.check_card(card))
            if result['status'] == 'live': live += 1
            elif result['status'] == 'dead': dead += 1
            else: unknown += 1
            
            if i % 5 == 0:
                msg.edit_text(f"🔄 {i+1}/{len(cards)}\n🔥 {live} | ❌ {dead} | ⚠️ {unknown}")
            time.sleep(load_settings().get('check_delay', 1))
        
        loop.close()
        save_cards([])
        msg.edit_text(f"✅ Complete!\n🔥 Live: {live}\n❌ Dead: {dead}\n⚠️ Unknown: {unknown}", parse_mode="Markdown")

    def mtxt_command(update, context):
        if update.effective_user.id not in ADMIN_IDS:
            update.message.reply_text("❌ Admin only!")
            return
    ...
    def handle_file(update, context):
        ...
        update.message.reply_text(f"✅ Added {len(cards)} cards! Total: {len(existing)}")

    def add_command(update, context):
        args = context.args
        if not args:
            update.message.reply_text("❌ Usage: /add 411111|12|2026|123")
            return
        
        cards = load_cards()
        cards.append(' '.join(args))
        save_cards(cards)
        update.message.reply_text(f"✅ Added! Total: {len(cards)}")
...
"""
    ...
    def live_command(update, context):
        ...
        if not live:
            update.message.reply_text("📭 No live cards!")
            return
    ...
    def dead_command(update, context):
        try:
        ...
        if not dead:
            update.message.reply_text("📭 No dead cards!")
            return
    ...
    def clear_command(update, context):
        if update.effective_user.id not in ADMIN_IDS:
            ...
            return
        save_cards([])
        update.message.reply_text("🗑️ Cleared!")
...
"""
    ...
    def set_delay(update, context):
        ...
        args = context.args
        if not args:
            update.message.reply_text("❌ Usage: /setdelay 1")
            return
        
        settings = load_settings()
        settings['check_delay'] = float(args[0])
        save_settings(settings)
        update.message.reply_text(f"✅ Delay: {args[0]}s")

    def toggle_proxy(update, context):
        if update.effective_user.id not in ADMIN_IDS:
            update.message.reply_text("❌ Admin only!")
            return
        ...
        save_settings(settings)
        update.message.reply_text(f"✅ Proxy {'enabled' if settings['use_proxy'] else 'disabled'}")
    ...
    def set_key(update, context):
        ...
        args = context.args
        if not args:
            update.message.reply_text("❌ Usage: /setkey pk_test_xxx")
            return
        
        settings = load_settings()
        settings['publishable_key'] = args[0]
        save_settings(settings)
        update.message.reply_text("✅ Key updated!")

    def button_handler(update, context):
        query = update.callback_query
        query.answer()
        if query.data == "check":
            query.message.reply_text("💳 Use: /chk 4111111111111111|12|2026|123")
    ...
    # ===== V13 MAIN =====
    def main():
        print("🔥 Starting LIVE CC Checker Bot (v13)...")
        create_files()
        ...
        try:
            ...
            dp.add_handler(CommandHandler("chk", chk_command))
            dp.add_handler(CommandHandler("mass", mass_command))
            dp.add_handler(CommandHandler("mtxt", mtxt_command))
            dp.add_handler(CommandHandler("add", add_command))
            dp.add_handler(CommandHandler("list", list_command))
            dp.add_handler(CommandHandler("stats", stats_command))
            dp.add_handler(CommandHandler("live", live_command))
            dp.add_handler(CommandHandler("dead", dead_command))
            dp.add_handler(CommandHandler("clear", clear_command))
            dp.add_handler(CommandHandler("settings", settings_command))
            dp.add_handler(CommandHandler("setdelay", set_delay))
            dp.add_handler(CommandHandler("toggleproxy", toggle_proxy))
...
"""
    ...
    # ===== V20+ MAIN =====
    def main():
        print("🔥 Starting LIVE CC Checker Bot (v20+)...")
        create_files()
